// Author:Melody.Hu
// Title:Chinese Style Mei Flower
...
float noise(vec2 st) {
    ...
    return mix(mix(random2D(i + vec2(0.0, 0.0)), random2D(i + vec2(1.0, 0.0)), u.x), mix(random2D(i + vec2(0.0, 1.0)), random2D(i + vec2(1.0, 1.0)), u.x), u.y);
}
...
vec4 flower(vec2 st, vec2 pos, float hue, float size, float index) {
    ...
    for(float i = 0.; i < 5.; i++) {
        ...
        for(float j = 0.; j < PI; j += 0.1) {
            ...
            petalAlpha *= petalNoise * 0.1 + 0.9;//alpha variation
            float saturate = remap(dis, 0.0, petalRadius, 0.7, 0.0) - petalNoise * 0.2;
            ...
            if(mod(j, 0.3) == 0.) {
                ...
                vec3 hsvPistil = vec3(pistilNoise * 0.2, 0.4, 0.6);
...
void main() {
    ...
    //tree
    vec4 treeColor = fractalTree(st, 0.14, PI * ((-0.72)), vec2(1.), 0.027, vec3(0.0), 0.0);

    //flower
    vec4 finalColor = mix(bgColor, treeColor, treeColor.a);
    vec4 flowerColor = vec4(0.);

    for(float i = 36.; i < 100.; i++) {
        vec2 branchPos = stack[int(i)].startPos;
        float flowerSize = random(i) * 0.02 + 0.02;
        
        if(mod(i, 2.) != 0.) {
            vec2 pos = st - branchPos - vec2((random(branchPos.x) - 0.5), (random(branchPos.y) - 0.5)) * 0.1;
            // float flowerHue = remap(random2D(st), 0., 1.0, 0.9, 1.);
            float flowerHue = u_color;
