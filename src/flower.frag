...
vec4 fractalTree(vec2 st, float length, float angle, vec2 startPos, float weight, vec3 treeColor, float alpha) {
    ...
    for(int i = 0; i < 200; i++) {
        ...
        if(mod(float(i), 2.) == 0.) {
            ...
            weight = b.weight * 0.9;
            float angle = b.angle + (random2D(startPos) - random(float(i))) * 0.3 * PI; //
            ...
            float angle1 = b.angle * sign(-b.angle) + random(float(i)) * 0.3 * PI;
            float angle2 = b.angle - random2D(startPos) * 0.24 * PI;
...
vec4 flower(vec2 st, vec2 pos, float hue, float size, float index) {
    ...
    //draw one flower
    for(float i = 0.; i < 5.; i++) {
        float rotateAngle = (0.2 + random(i) * 0.1) * 2. * PI; //petal rotation//random(i)
        ...
        for(float j = 0.; j < PI; j += 0.1) {
            ...
            if(mod(j, 0.3) == 0.) {
                ...
                vec3 hsvPistil = vec3(pistilNoise * 0.2, pistilNoise, 0.7);
...
void main() {
    ...
    //tree
    vec4 treeColor = fractalTree(st, 0.14, PI * ((-0.75) + (u_angle - 0.5) * 0.1), vec2(1.), 0.027, vec3(0.0), 0.0);
